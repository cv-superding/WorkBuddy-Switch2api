...
/** 反代用量统计（服务没跑也能读历史）。 */
export function getProxyUsage(): Promise<ProxyUsage> {
  return call("get_proxy_usage");
}
...
export function setAccountGroup(
  accountId: string,
  group: string,
): Promise<{ ok: boolean; id: string; group: string }> {
  return call("set_account_group", { accountId, group });
}
